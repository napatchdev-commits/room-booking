...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1. Fetch Rooms Summary
    const { data: allRooms } = await supabase.from('rooms').select('id, status, price_per_night, room_type_id');
    const totalRooms = allRooms?.length || 0;
    const maintenanceRooms = allRooms?.filter((r) => r.status === 'maintenance').length || 0;

    // 2. Fetch Active Bookings for Today Occupancy
    const { data: activeTodayBookings } = await supabase
      .from('bookings')
    ...
    // 3. Today's Check-ins, Check-outs, Bookings
    ...
    const { data: todayNewBookings } = await supabase
      .from('bookings')
      .select('id, net_total')
      .gte('created_at', `${todayStr}T00:00:00.000Z`);

    // 4. Financial Totals
    const { data: allValidBookings } = await supabase
      .from('bookings')
